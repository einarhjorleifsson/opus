...
**The published parquet is no longer self-describing, and that was not a decision.** The previous archive embedded `obus:file` and `obus:fields` in the parquet footer, so a detached file carried its own field descriptions and `duckdbfs::open_dataset()` alone could surface them. `archive_06_consolidate.R` calls `arrow::write_parquet()` and embeds nothing but `ARROW:schema`, so that metadata now lives *only* in the sidecar catalog — which works well (`ATTACH` over https, joins to `enum_labels`) but does not travel with a copied file. DuckDB's `COPY … (FORMAT PARQUET, KV_METADATA {…})` would restore it in the same statement that writes the file; see TODO.md.
