...
**Latest:** 2026-08-17 — Root-caused and fixed the TRUE/FALSE archive-integrity bug flagged at the end of the previous session: `read.delim()` in both XML-parsing scripts auto-guessed any file-locally-all-T/F column as `logical`, silently destroying the real `T`/`F` text before WSDL-based casting ever ran (confirmed clean in raw XML — 100% opus's own pipeline bug, not an ICES data issue). An exhaustive re-sweep (not just the previously-known columns) found 7 affected fields, one more than documented (`CA.SpeciesCodeType`), ~2.12M rows total, never LT. Fixed both parsers, rebuilt the full 4-table archive from raw XML (counts match the known-good baseline exactly, zero new failure modes), re-split legacy/new parquet, and re-verified zero corruption remains. Also checked whether this had contaminated the previous session's HH-vs-LT `Rigging` match-rate finding (95.33%) — it hadn't; recomputed at 95.28%, same `FB`/`FW`-swap composition, denominator noise only. Same day: removed the 6 dead three-phase-bootstrap `R/` files (verdict: remove, not repurpose — all six needed an already-existing curated YAML as input, the catch-22 the current `spec_00`-`03` pipeline replaced them to avoid); `devtools::document()` auto-cleaned the 6 orphaned man pages, `NAMESPACE` correctly untouched (hand-frozen, never had them). See AGENTS.md's 2026-08-17 entries for full detail. Everything else from the 2026-08-16 pre-staging review backlog below remains open.
...
Surfaced by a three-part review (data-dict example alignment, skill-read/skill-create audit, full structural review) run before staging the day's work. Two items from this review were fixed the same day (`AgeSource`/`AgePrepMet` vocab-key redirect, three CA sampling-flag columns) — see AGENTS.md's "Pre-staging review" entry. Two more were resolved 2026-08-17 (below). Everything else is still open:

- [x] ~~**New archive-integrity bug**: "T"/"F" values in several enum columns stored as literal text `"TRUE"`/`"FALSE"` instead of `"T"`/`"F"` (`HL.SpeciesCodeType` alone: 1.5M of 13.75M rows). Hits HH/HL/CA, never LT. Needs its own root-cause + rebuild cycle, same scale as the `sentinel_replacement_data_loss` fix — don't fold into a smaller task.~~ — fixed 2026-08-17: `read.delim()`'s default type-guessing was silently coercing file-locally-all-T/F columns to `logical` before WSDL casting ran. Full root-cause, fix, and rebuild in AGENTS.md's 2026-08-17 entry; exhaustive re-check found one more affected column than originally listed here (`CA.SpeciesCodeType`).
- [x] ~~Remove or repurpose 6 dead `R/` files (`op_apply_curated_spec.R`, `op_audit_yaml_phase2_mismatch.R`, `op_build_final_yaml.R`, `op_check_type_mismatch.R`, `op_enrich_stage2_yaml.R`, `op_minimal_yaml.R`) — superseded, zero callers, but still `@export`-tagged with live man pages.~~ — removed 2026-08-17 (verdict: remove, not repurpose — all six required an already-existing curated YAML as input, the exact catch-22 the current `spec_00`-`03` pipeline was built to avoid). Full evidence in AGENTS.md's 2026-08-17 entry.
