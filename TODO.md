...
Surfaced by a three-part review (data-dict example alignment, skill-read/skill-create audit, full structural review) run before staging the day's work. Two items from this review were fixed the same day (`AgeSource`/`AgePrepMet` vocab-key redirect, three CA sampling-flag columns) — see AGENTS.md's "Pre-staging review" entry. A third was root-caused and fixed 2026-08-17 (below). Everything else is still open:
