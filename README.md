...
**not** a data-quality QC log (that’s downstream in obus/imbus) - Use
when: “Why did validation fail?” to determine if it’s a spec problem or
submission error
...
- **obus** — Downstream validation and submission-tracking package
  (depends on opus specs)
- **imbus** — EU research project; uses opus to track metadata issues
  with ICES
- **icesDatras** — R-native exploration tool; shares seeding methodology
  with opus
...
- Tier 1 dictionaries (HH, HL, CA, LT) finalized after full archive
  validation
...
- **Never edit `.yaml` files by hand** — they’re derived from
  `data-raw/` scripts
- Scripts in `data-raw/` are the source of truth (seeding, curation,
  validation)
